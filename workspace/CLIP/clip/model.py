...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
         # 获取注意力输出和注意力权重
        attn_output, attn_weights = self.attn(x, x, x, need_weights=True, average_attn_weights=True, attn_mask=self.attn_mask)
    ...
    def forward(self, x: torch.Tensor):
        ...
        o, attn_weights = self.attention(self.ln_1(x))
        # 存储注意力权重以供后续使用
        x = x + o
        x = x + self.mlp(self.ln_2(x))
        return x
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        ...
        # if logits_per_image.dim() > 2:
        #     logits_per_image = logits_per_image.view(logits_per_image.size(0), -1)
