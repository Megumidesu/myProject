...
class TransformerClassifier(nn.Module):
    def __init__(self, attr_num, attr_words, dim=768, pretrain_path='/root/autodl-tmp/vit_model/jx_vit_base_p16_224-80ecf9dd.pth'):
        ...
        self.text = torch.cat([clip.tokenize(f"This is an photo of {a}, a type of airplane") for a in attr_words]).to(device)
    ...
    def forward(self,videos,ViT_model):
        ...
        imgs=videos.view(-1, channels, height, width) #[batch_size*num_frames, channels, height, width]
        #imgs=videos[:,0,:,:,:]
        #CLIP 提取视频帧特征
        for img in imgs:
            img=img.unsqueeze(0) #[1, channels, height, width]
            ViT_features.append(ViT_model.encode_image(img).squeeze(0)) #[1, token_num, 512]
            #图像特征
        ViT_image_features=torch.stack(ViT_features).to(device).float() #[batch*frame, token_num, 512]
        ...
        ViT_image_features=self.visual_embed(torch.mean(ViT_image_features,dim=1)) #[batch, token_num, 768] 平均了frame并且嵌入768
        text_features = ViT_model.encode_text(self.text).to(device).float()  #[text_num(5), 512]
