...
def batch_trainer(epoch, model,ViT_model, train_loader, criterion, optimizer):
    ...
    for step, (imgs, gt_label, imgname, label_v) in enumerate(train_loader):
        ...
        imgs, gt_label = imgs.cuda(), gt_label.cuda() #[batchsize, frame, depth, height, width]
    ...
    print(f'Epoch {epoch}, LR {lr}, Train_Time {time.time() - epoch_time:.2f}s, Loss: {loss_meter.avg:.4f}, img_num:{img_count*batch_num}')
    img_count=0
