...
def get_multiAttr_metrics(gt_label, preds_probs, threshold=0.45):
    ...
    class_id = []
    for row in gt_label:
        class_id.append(int(np.where(row == 1)[0]))

    cls_ap = [0,0,0,0,0]
    cls_num = [0,0,0,0,0]
    for cls, prec in zip(class_id, instance_prec):
        cls_ap[cls] += prec 
        cls_num[cls] += 1
    for i, num in enumerate(cls_num) :
        cls_ap[i] /= num
    
    result.instance_per_AP = cls_ap
    result.instance_mAP = np.mean(cls_ap)
    #################################################################################
